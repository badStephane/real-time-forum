...
function openChatWindow(user) {
  if (sessionStorage.getItem("nickname") === user.nickname) {
    console.log("You cannot open a chat window with yourself.");
    return;
  }
  console.log("The user is: ", user);
  hideBubbleGif(user.nickname);
  console.log("Opening chat window with user: " + user.nickname);

  // Set the current receiver to the user that was clicked
  CurrentReceiver = user.nickname;
  ...
  getChatHistory(user, (chatHistory) => {
    ...
    chatHeader.textContent = "Chatting with " + user.nickname;
    ...
    history.chathistory.reverse().forEach((message) => {
      ...
      messageContainer.className =
        message.from === sessionStorage.getItem("nickname")
          ? "message sent"
          : "message received";
...
socket.addEventListener("message", (event) => {
  ...
  if (message.type === "message") {
    ...
    if (chatContainer) {
      ...
      chatContainer.scrollTop = chatContainer.scrollHeight;
    } else if (message.from != sessionStorage.getItem("nickname")) {
      // Show bubble.gif when the chat window is not open and the message is not from the current user
      //console.log("Debug: ", message.text, message.type, message.nickname);
      showBubbleGif(message.nickname);
    }
  }
  if (message.type === "typing") {
    if (
      message.data.to === sessionStorage.getItem("nickname") &&
