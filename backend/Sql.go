...
func CheckIfEmailExist(db *sql.DB, email string) bool {
	...
	row := db.QueryRow("select id,nickname,passwrd,email,created_at from user where email = ?", email)
...
func CheckIfUserExist(db *sql.DB, nickname string) bool {
	var expected_user User

	row := db.QueryRow("select id,nickname,passwrd,email,created_at from user where nickname = ?", nickname)
	err := row.Scan(&expected_user.ID, &expected_user.Nickname, &expected_user.Password, &expected_user.Email, &expected_user.CreatedAt)
	return err == nil
}

// Function to register a user in the database
func RegisterUser(db *sql.DB, nickname string, age int, gender string, fname string, lname string, email string, password string) string {

	// Insert the user into the database
	_, err := db.Exec("insert into user(nickname,passwrd,email,fname,lname,age,gender,created_at) values(?,?,?,?,?,?,?,?)", nickname, password, email, fname, lname, age, gender, time.Now().Local().Format(time_format))
	if err != nil {
		log.Fatal(err)
	}
	return "200 OK"
...
func CheckIfPasswordIsCorrect(db *sql.DB, nickname string, password string) bool {
	var expected_user User
	...
	if ComparePasswords([]byte(expected_user.Password), password) {
		return true
	}
...
func GetLatestPosts(db *sql.DB) ([]Post, error) {
	...
	query := `SELECT p.id, p.user_id, p.title, p.content, p.created_at, u.nickname, c.category_name
				FROM post p
	...
	for rows.Next() {
		var p Post
		err := rows.Scan(&p.ID, &p.UserID, &p.Title, &p.Content, &p.CreatedAt, &p.NickName, &p.CategoryName) // Add the CategoryName field
		if err != nil {
			return nil, err
		}
	...
	// update posts with correct nickname
	for i := 0; i < len(posts); i++ {
		temp, _ := GetUser(db, posts[i].UserID)
		temp2 := string(temp.Nickname)
		posts[i].NickName = temp2
	}
...
func GetUser(db *sql.DB, userID int) (User, error) {
	var user User
	row := db.QueryRow("select id,nickname,passwrd,email,created_at from user where id = ?", userID)
	err := row.Scan(&user.ID, &user.Nickname, &user.Password, &user.Email, &user.CreatedAt)
	if err != nil {
		return User{}, err
	}
	return user, nil
...
// Get user depending on nickname
func GetUserID(db *sql.DB, nickname string) string {
	// Prepare the SQL query to retrieve the user ID based on the nickname
	query := "SELECT id FROM user WHERE nickname = ?"

	// Execute the query and retrieve the user ID
	var userID string
	err := db.QueryRow(query, nickname).Scan(&userID)
	if err != nil {
...
// Get nickname depending on userID
func GetNicknameFromId(db *sql.DB, id string) string {
	// Prepare the SQL query to retrieve the user ID based on the nickname
...
func GetChatHistory(user string, from string, offset int) []Message {
	...
	for rows.Next() {
		...
		toUser = GetNicknameFromId(db, toUser)
		fromUser = GetNicknameFromId(db, fromUser)
...
func GetAllUsers(db *sql.DB) ([]User, error) {
	rows, err := db.Query("SELECT id,nickname,passwrd,email,created_at FROM user")
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		err := rows.Scan(&user.ID, &user.Nickname, &user.Password, &user.Email, &createdAt)
		if err != nil {
			return nil, err
		}
...
func AddPost(db *sql.DB, title string, content string, category string, nickname string) {
	// Get user ID
	userID := GetUserID(db, nickname)

	// Insert post into database
	_, err := db.Exec("insert into post(user_id,title,content,created_at,updated_at) values(?,?,?,?,?)", userID, title, content, time.Now().Format(time_format), time.Now().Format(time_format))
	if err != nil {
		// Handle error
		fmt.Printf("Server >> Error adding post to database: %s ", err)
	}
	fmt.Println("Server >> Post added to database")
}

func CreateCategory(db *sql.DB, category string, nickname string) {
	...
	fmt.Println("Server >> Category added to database")
}

func GetCommentsByPostTitle(db *sql.DB, postTitle string) ([]Comment, error) {
	query := `
        SELECT comment.id, comment.user_id, comment.post_id, user.nickname, comment.content, comment.created_at, comment.updated_at
        FROM post
	...
	for rows.Next() {
		var comment Comment
		err := rows.Scan(&comment.ID, &comment.UserID, &comment.PostID, &comment.Nickname, &comment.Content, &comment.CreatedAt, &comment.UpdatedAt)
		if err != nil {
			return nil, err
		}
...
func InsertComment(db *sql.DB, postTitle string, nickname string, content string) {
	// Get post ID
	...
	_, err := db.Exec("insert into comment(user_id,post_id,content,created_at,updated_at) values(?,?,?,?,?)", userID, postID, postTitle, time.Now().Format(time_format), time.Now().Format(time_format))
	if err != nil {
		// Handle error
		fmt.Printf("Server >> Error adding comment to database: %s ", err)
	}
	fmt.Println("Server >> Comment added to database")
}
...
func AddPostCategoryRelation(db *sql.DB, postTitle string, categoryName string) {
	...
	fmt.Println("Server >> Post Category relation added to database")
}

func GetNicknamebyEmail(db *sql.DB, email string) string {
	fmt.Println("Getting nickname from email")
	var nickname string

	err := db.QueryRow("SELECT nickname FROM USER user WHERE email = ?", email).Scan(&nickname)
	if err != nil {
		fmt.Println("nickname cannot be retrieved")
	}
	return nickname
}
func GetPostsByCategory(db *sql.DB, category_name string) ([]Post, error) {
	...
	// sort posts by date and time created (newest first)à
	sort.Slice(posts, func(i, j int) bool {
		return posts[i].CreatedAt.After(posts[j].CreatedAt)
	})
...
// Fonction pour créer une session dans la base de données
func InsertSessionIntoDB(db *sql.DB, nickname string, sessionToken string, expiredTime time.Time) error {
	// Préparer la requête d'insertion
	query := "INSERT INTO sessions (nickname, cookie, expired_time) VALUES (?, ?, ?)"

	// Exécuter la requête d'insertion
	_, err := db.Exec(query, nickname, sessionToken, expiredTime)
	if err != nil {
		return err
	}
...
func DeleteSessionByCookie(db *sql.DB, cookie string) error {
	_, err := db.Exec("DELETE FROM sessions WHERE cookie = ?", cookie)
	return err
}

func RemoveSessionFromDatabase(db *sql.DB, sessionToken string) error {
	// Prépare la requête SQL pour supprimer la session de la base de données.
	query := "DELETE FROM sessions WHERE cookie = ?"

	// Exécute la requête SQL.
	_, err := db.Exec(query, sessionToken)
	if err != nil {
		// En cas d'erreur, renvoie l'erreur.
